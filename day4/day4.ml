...
let increment_map map =
  List.fold_left ~init:map
                 ~f:(Map.update ~f:(Option.value_map ~default:1 ~f:succ))
...
let guard_sleep_times =
  ...
  List.fold_left
    ...
    ~f:(fun map {start; length ; num} ->
      ...
      Map.update map num
        ~f:(function
          | None                  -> length, increment_map empty_map min_list
          | Some (num_slept, map) -> num_slept + length, increment_map map min_list))
